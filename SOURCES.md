...
| grok-acp | first-party (this repo) | first-party | MIT | Worker skill: run the local Grok Build CLI as an SDLC builder over ACP, with label registry and resume. **Ships `scripts/grok_acp.py`** (stdlib Python ACP client) — the one operator-approved exception to "no scripts" (2026-09-21). Runs Grok with maximum permissions by operator order. Operator opt-in per ask. |
