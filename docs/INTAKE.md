...
reads the change that adds or edits it (no tokens, no `scripts/` files;
a fenced shell recipe copied from `skills/tracker-sdlc/adapters/local.md`
is allowed and gets the same read).
